...
def afficher_score_et_taille(joueur, score, vies, temps_debut, nb_poissons_max):
    ...
    if bonus_actif and time.time() - bonus_timer < 10:  # Durée de 10 secondes pour le bonus x2
        screen.blit(bonus_emoji_image, (x_score_x2, y_score_x2))
    elif bonus_actif and time.time() - bonus_timer >= 10:
        bonus_actif = False  # Désactive le bonus après 10 secondes
...
class ParametresPartie:
    def __init__(self):
        ...
        self.vitesse_bonus = 3
        self.duree_bonus = 20  # Réduit à 10 secondes
        ...
        self.taux_apparition_bonus = 0.02  # 2% de chance par frame
        
        # Paramètres d'apparition des poissons
        self.taux_apparition_poisson = 0.04  # Augmenté à 10% de chance par frame
        self.max_poissons = 260
...
class Partie:
    def __init__(self, parametres=None):
        global bonus_actif, bonus_timer  # Ajout de la déclaration globale
    ...
    def gerer_mouvements_joueur(self):
        ...
        if touches[pygame.K_LEFT] and self.joueur.x > 0:
            self.joueur.x -= self.parametres.vitesse_joueur
            if self.joueur.derniere_direction >= 0:  # Si on allait à droite ou état initial
        ...
        if touches[pygame.K_RIGHT] and self.joueur.x < LARGEUR - self.joueur.taille:
            self.joueur.x += self.parametres.vitesse_joueur
            if self.joueur.derniere_direction <= 0:  # Si on allait à gauche ou état initial
        ...
        # Si l'image a changé, met à jour la taille
    ...
    def generer_requin(self, temps_actuel):
        if temps_actuel - self.dernier_requin >= self.intervalle_requin:
            # Force le requin à apparaître de la droite
            x = LARGEUR
            y = random.randint(HAUTEUR_BANDE, HAUTEUR - 100)
            requin = Requin(x, y, direction=-1)  # Direction toujours vers la gauche
        ...
        for poisson in self.poissons[:]:
            ...
            if self.joueur.rect.colliderect(poisson.rect):
                if isinstance(poisson, Requin):
                    if self.joueur.vies > 1 and not self.joueur.invulnerable:
                        self.joueur.vies -= 1
                    ...
                    if poisson.score < self.joueur.score:
                        ...
                        if self.joueur.vies > 1:
                            self.joueur.vies -= 1
                            self.joueur.invulnerable = True
                            self.joueur.invulnerable_timer = time.time()

    def gerer_bonus(self, temps_actuel):
        # Gestion de la durée du bonus score x2
        if self.bonus_timer:
            temps_ecoule = temps_actuel - self.bonus_timer
            if temps_ecoule > self.parametres.duree_bonus:
                self.bonus_actif = False
                self.bonus_timer = 0  # Réinitialise le timer

         # Création d'un nouveau bonus toutes les 20 secondes
        if (temps_actuel - self.dernier_bonus >= self.parametres.intervalle_bonus and 
            ...
            self.dernier_bonus = temps_actuel + random.uniform(-5, 5)  # Ajoute un peu d'aléatoire
    ...
    def boucle_principale(self):
        ...
        # Réinitialisation des vitesses après la fin des bonus
            if self.joueur.dernier_bonus_buff and temps_actuel - self.joueur.dernier_bonus_buff > self.parametres.duree_bonus:
                vitesse_poisson_autre = self.parametres.vitesse_joueur  # Retour à la vitesse normale
            ...
            if self.joueur.dernier_bonus_nerf and temps_actuel - self.joueur.dernier_bonus_nerf > self.parametres.duree_bonus:
                vitesse_poisson_autre = self.parametres.vitesse_poisson  # Retour à la vitesse normale
            ...
            for poisson in self.poissons:
                poisson.dessiner()
           # On passe correctement le nombre actuel de poissons et le maximum
            afficher_score_et_taille(self.joueur, self.joueur.score, 
                               self.joueur.vies, self.temps_debut, 
                               f"{len(self.poissons)}/{self.parametres.max_poissons}")  # Modification ici
...
def jeu():
    ...
    params.vies_initiales = 3  # Exemple de modification
    vitesse_poisson_autre = 3  
