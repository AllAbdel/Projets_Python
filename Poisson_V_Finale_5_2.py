...
class Poisson:
    def __init__(self, x, y, taille, score, vies, direction=1, est_joueur=False):
        ...
        if est_joueur:
            ...
            self.image = self.image_droite  # Par défaut vers la droite
        else:
            # Initialisation de l'image d'un poisson ennemi
            self.vitesse = random.randint(1, 3)
            self.vitesse_initiale = self.vitesse
            if direction == 1:
                self.image = random.choice(images_poissons_gauche)
            else:
                self.image = random.choice(images_poissons_droite)
    ...
    def dessiner(self):
        ...
        texte = police.render(str(self.score), True, BLANC)
        text_rect = texte.get_rect(center=self.rect.center)
        screen.blit(texte, text_rect)

    def deplacer(self):
        # Utilisation de self.vitesse, qui peut être modifiée dans `gerer_bonus`
...
class ObjetBonusAvance:
    def __init__(self):
        ...
        self.bonus_images = {
            ...
            "freeze": pygame.transform.scale(pygame.image.load(r"img\tres\freeze.png"), (30, 30)),
...
def gestion_bonus_avance(joueur, objet_bonus):
    ...
    if objet_bonus.type_bonus == "freeze":
        bonus_timer = time.time()
...
def afficher_score_et_taille(joueur, score, vies, temps_debut, nb_poissons_max):
    ...
    screen.blit(score_emoji, (-25, -42))
    police = pygame.font.SysFont("impact", 20, bold=False)
    screen.blit(police.render(f"{score}", True, gold_light), (85, 33))

    vies_x = LARGEUR // 5
    for i in range(vies):
        screen.blit(coeur_image, (vies_x + (i * 20), HAUTEUR_BANDE // 2.1))
    screen.blit(poisson_emoji, (LARGEUR // 4, HAUTEUR_BANDE // 8))
    screen.blit(police.render(f": {nb_poissons_max}", True, BLANC), (LARGEUR // 3.65, HAUTEUR_BANDE // 7))
    ...
    if joueur.invulnerable:
        ...
        if temps_restant > 0:
            ...
            screen.blit(bouclier_emoji, (LARGEUR // 1.87, HAUTEUR_BANDE // 9.5))
            text_rect = inv_texte.get_rect(center=(LARGEUR // 1.85, HAUTEUR_BANDE // 1.6))
            screen.blit(inv_texte, text_rect)

    # Position et affichage des bonus existants et nouveaux
    ...
    if bonus_actif and time.time() - bonus_timer < 20:
        screen.blit(bonus_emoji_image, (x_score_x2, y_score_x2))
    elif bonus_actif and time.time() - bonus_timer >= 20:
        bonus_actif = False
    
    # Nouveaux bonus : freeze, attraction, super attraction
    x_freeze, y_freeze = int(LARGEUR * 0.80), int(HAUTEUR_BANDE * 0.3)
    #x_attraction, y_attraction = int(LARGEUR * 0.85), int(HAUTEUR_BANDE * 0.3)
    #x_super_attraction, y_super_attraction = int(LARGEUR * 0.90), int(HAUTEUR_BANDE * 0.3)
    ...
    """
    # Affichage du bonus "attraction"
    if joueur.dernier_bonus_attraction and time.time() - joueur.dernier_bonus_attraction < 20:
        attraction_image = pygame.transform.scale(pygame.image.load(r"img\tres\aimant.png"), (32, 32))
        screen.blit(attraction_image, (x_attraction, y_attraction))
    
    # Affichage du bonus "super attraction"
    if joueur.dernier_bonus_super_attraction and time.time() - joueur.dernier_bonus_super_attraction < 20:
        super_attraction_image = pygame.transform.scale(pygame.image.load(r"img\tres\superaimant.png"), (32, 32))
        screen.blit(super_attraction_image, (x_super_attraction, y_super_attraction))
    """
    # Cadre des bonus
    cadre_largeur = int(LARGEUR * 0.25)  # élargir le cadre pour inclure les nouveaux bonus
...
class Partie:
    ...
    def afficher_ecran_fin(self, victoire):
        ...
        while running:
            overlay = pygame.Surface((300, 500), pygame.SRCALPHA)
            overlay.fill((125, 125, 125, 128))  # Noir semi-transparent (128 est l'alpha)
            self.screen.blit(overlay, (0, 0))
            
            # Affichage du message de victoire/défaite
            message = "VICTOIRE !" if victoire else "DÉFAITE..."
            couleur = (0,0,255) if victoire else ROUGE
    ...
    def gerer_bonus(self, temps_actuel):
        ...
        # Gestion des vitesses (buff/nerf) pour le joueur
        ...
        # Gestion du "nerf" qui ralentit les ennemis
        ...
        # Gestion du bonus freeze qui arrête les ennemis
        if self.joueur.dernier_bonus_freeze and temps_actuel - self.joueur.dernier_bonus_freeze <= self.parametres.duree_bonus:
            # Si le bonus "freeze" est actif, on arrête tous les poissons
            for poisson in self.poissons:
                poisson.vitesse = 0
        else:
            # Sinon, on remet leur vitesse normale
            for poisson in self.poissons:
                poisson.vitesse = poisson.vitesse_initiale  # Utilise la vitesse initiale définie pour chaque poisson
            if self.joueur.dernier_bonus_freeze and temps_actuel - self.joueur.dernier_bonus_freeze > self.parametres.duree_bonus:
    ...
    def boucle_principale(self):
        ...
        Partie.vitesse_initiale_poisson = self.parametres.vitesse_poisson_autre
        ...
        while True:
            ...
            if self.joueur.score >= len(self.poissons) + 20:
...
class ParametresPartie:
    def __init__(self):
        ...
        self.taux_apparition_poisson_def = 0.03 # Pour la réinitialisation
...
if __name__ == "__main__":
    parms = ParametresPartie()
    parms.intervalle_requin_max = 5
    parms.max_poissons = 50
    parms.vitesse_actuelle_joueur = 5
    parms.intervalle_bonus = 1
    Partie.main()