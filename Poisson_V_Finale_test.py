...
VAGUE_GTOD_IMAGE = "img/event/vagueGtoD.png"
VAGUE_DTOG_IMAGE = "img/event/vagueDtoG.png"
...
if self.invulnerable:
# Augmenter la durée du clignotement à 1 seconde
if (time.time() - self.invulnerable_timer) % 1 < 0.75: # Clignotement 75% du temps
# Créer une surface rougâtre
...
# Dessiner la surface rougâtre avec un léger agrandissement
...
if not isinstance(poisson, Requin): # Optionnel : ne pas geler les requins
poisson.freeze = True
...
self.zone_sure = random.randint(HAUTEUR_BANDE + 100, HAUTEUR - 100)
self.epaisseur_zone = 80
...
# Afficher les vagues selon la direction
for poisson in self.poissons_vague:
if poisson.direction > 0:
screen.blit(self.vague_gtod, (poisson.x, poisson.y))
else:
screen.blit(self.vague_dtog, (poisson.x, poisson.y))

class PoissonVague(Poisson):
def __init__(self, x, y, direction):
super().__init__(x, y, taille=40, score=float('inf'), vies=0, direction=direction)
self.vitesse = 10
# Charger l'image appropriée selon la direction
...
poissons_vague = []
espacement = 60 # Espacement entre les poissons

# Générer des poissons pour la zone supérieure
for y in range(HAUTEUR_BANDE, alerte_vague.zone_sure - alerte_vague.epaisseur_zone//2, espacement):
# Poissons allant de gauche à droite
poissons_vague.append(PoissonVague(-40, y, 1))
# Poissons allant de droite à gauche
poissons_vague.append(PoissonVague(LARGEUR, y, -1))

# Générer des poissons pour la zone inférieure
for y in range(alerte_vague.zone_sure + alerte_vague.epaisseur_zone//2, HAUTEUR - 40, espacement):
# Poissons allant de gauche à droite
poissons_vague.append(PoissonVague(-40, y, 1))
# Poissons allant de droite à gauche
poissons_vague.append(PoissonVague(LARGEUR, y, -1))
...
self.intervalle_bonus = 20 # Nouveau paramètre : intervalle entre les bonus
...
norme = (dx**2 + dy**2)**0.5
if norme > 0:
dx = dx/norme * 240 # Répulsion de 200 pixels (multiplié par 4)
dy = dy/norme * 240
joueur.x = min(max(joueur.x + dx, 0), LARGEUR - joueur.taille)
joueur.y = min(max(joueur.y + dy, HAUTEUR_BANDE), HAUTEUR - joueur.taille)
joueur.rect.x = joueur.x
joueur.rect.y = joueur.y
elif distance <= self.rayon_repulsion:
dx = centre_joueur[0] - centre_bombe[0]
dy = centre_joueur[1] - centre_bombe[1]
norme = (dx**2 + dy**2)**0.5
if norme > 0:
dx = dx/norme * 200 # Répulsion de 200 pixels (multiplié par 4)
dy = dy/norme * 200
joueur.x = min(max(joueur.x + dx, 0), LARGEUR - joueur.taille)
joueur.y = min(max(joueur.y + dy, HAUTEUR_BANDE), HAUTEUR - joueur.taille)
joueur.rect.x = joueur.x
...
if self.explosion:
self.explosion.dessiner(screen)
class Particule:
...
self.intervalle_goldfish = 25 # 15 secondes entre chaque apparition
self.bombes = []
self.dernier_spawn_bombe = time.time()
self.intervalle_bombe = 15 # Une bombe toutes les 20 secondes
self.alerte_vague = None
self.dernier_event_vague = time.time()
self.intervalle_vague = 30 # Une vague toutes les 30 secondes
...
nouvelle_taille = min(self.joueur.taille + gain // 2, 75) # Limite la taille à 75
self.joueur.taille = nouvelle_taille
self.joueur.image = pygame.transform.scale(
self.joueur.image_droite if self.joueur.derniere_direction >= 0 else self.joueur.image_gauche,
(nouvelle_taille, nouvelle_taille)
)
self.joueur.rect = self.joueur.image.get_rect(topleft=(self.joueur.x, self.joueur.y))
...
if temps_actuel - poisson.freeze_timer >= poisson.freeze_duration:
