...
class MenuPause:
    ...
    def afficher(self, screen):
        ...
        surface_pause.fill((128, 128, 128, 128))  # Gris plus transparent
...
class AlerteVague:
    def __init__(self):
        ...
        self.zone_sure = random.randint(HAUTEUR_BANDE + 90, HAUTEUR - 90)  # Pour assurer l'espace pour deux poissons
        self.epaisseur_zone = 90  # Pour deux espaces de 45 pixels
    ...
    def dessiner(self, screen, joueur_x, joueur_y):
        if not self.vague_active:
            ...
            # Dessiner les lignes rouges pour chaque position de poisson vague
            surface_zones = pygame.Surface((LARGEUR, HAUTEUR), pygame.SRCALPHA)
            
            for poisson in self.poissons_vague:
                # Dessiner une ligne rouge semi-transparente pour chaque poisson
            ...
            # Zone sûre (sans ligne rouge)
            pygame.draw.rect(surface_zones, (0, 255, 0, 30), 
                           (0, self.zone_sure - self.epaisseur_zone//2, 
                            LARGEUR, self.epaisseur_zone))
...
class PoissonVague(Poisson):
    def __init__(self, x, y, direction):
        ...
        self.vitesse = 10
        self.image = pygame.transform.scale(pygame.image.load(r"img\event\vagueGtoD.png"), (45, 45))

def generer_poissons_vague(alerte_vague):
    poissons_vague = []
    espacement = 45

    # Déterminer les limites de la zone sûre
    zone_sure_bas = alerte_vague.zone_sure - 45
    zone_sure_haut = alerte_vague.zone_sure + 45

    # Générer des poissons pour TOUTE la hauteur
    positions_utilisees = set()  # Pour éviter les doublons
    ...
    for y in range(HAUTEUR_BANDE, HAUTEUR - 45, espacement):
        ...
        if y not in positions_utilisees:
            poissons_vague.append(PoissonVague(-40, y, 1))
            positions_utilisees.add(y)
...
class ParametresPartie:
    def __init__(self):
        ...
        self.taux_apparition_poisson = 0.02  # Augmenté à 3% de chance par frame
...
class AnimationRepulsion:
    def __init__(self, joueur, dx, dy, force, duree=20):
        ...
        self.onde = OndeChoc(joueur.x + joueur.taille/2, joueur.y + joueur.taille/2)
...
class Partie:
    def __init__(self, parametres=None):
        ...
        self.intervalle_peche = 50  # Par exemple, un événement canne à pêche toutes les 20 secondes
        self.dernier_goldfish = time.time()
        self.intervalle_goldfish = 17  # 15 secondes entre chaque apparition
        self.bombes = []
    ...
    def manger_goldfish(self, goldfish):
        if goldfish in self.poissons:
            ...
            pygame.time.delay(1000)  # Attendre 2 secondes
